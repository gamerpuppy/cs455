...
public class MetadataMapper extends Mapper<LongWritable, Text, Text, IntWritable> {

    // Q1
    private HashMap<String, Integer> q1Map = new HashMap<>();
    ...
    private void mapQ1(String artistId, String artistName) {
        ...
        int count = this.q1Map.getOrDefault(key, 0);
        this.q1Map.put(key, count + 1);
    }

    private void cleanupQ1(Context context) throws IOException, InterruptedException {
        for (String q1Key : this.q1Map.keySet()) {
            int count = this.q1Map.get(q1Key);
            context.write(new Text(q1Key), new IntWritable(count));
        }
    }

    private void mapQ3(String songTitle, Double hotness) {
    ...
    private void cleanupQ3(Context context) throws IOException, InterruptedException {
        if(maxHotnessTitle != null && maxHotness != null)
